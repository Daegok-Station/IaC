...
sudo mkdir -p /home/ec2-user/jenkins-docker
...
# 6. Docker 이미지 빌드
sudo docker build -t my-jenkins:custom .
...
# 8. Jenkins CLI 다운로드
sudo docker exec jenkins curl -sSL http://localhost:8080/jnlpJars/jenkins-cli.jar -o /var/jenkins_home/jenkins-cli.jar
...
sudo docker exec jenkins java -jar /var/jenkins_home/jenkins-cli.jar -s http://localhost:8080 create-job my-pipeline < /home/ec2-user/jenkins-docker/my-pipeline.xml

sleep 30

# 11. 최초 빌드 실행
sudo docker exec jenkins java -jar /var/jenkins_home/jenkins-cli.jar -s http://localhost:8080 build my-pipeline
